...
The module also includes a complete Item Piles integration for Black Flag, covering currencies, merchant prices, containers, item filters, stacking rules, and vault colors. Unlike the previous adapter, this integration correctly respects `system.price.denomination`. Registered Black Flag currency items already store the GP value per unit expected by Item Piles, so their conversion values are used directly.
